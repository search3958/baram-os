...
static void window_update_caches(window_t *win) {
  ...
  uint32_t theme;
  if (is_dark) theme = (win == &g_windows[g_active_window_index]) ? 0xFF1E1E1E : 0xFF333333;
  else theme = (win == &g_windows[g_active_window_index]) ? 0xFFF5F5F5 : 0xFFE0E0E0;
      ...
      for (int dy = -i_r; dy <= i_r; dy++) {
        for (int dx = -i_r; dx <= i_r; dx++) {
          ...
          if (px >= 0 && px < fw && py >= 0 && py < fh) {
            ...
            if (alpha_f > 0.0f) frame_l.buffer[py * fw + px] = blend_colors(frame_l.buffer[py * fw + px], colors[k], (uint8_t)(alpha_f * 255));
          }
        }
      }
    }
  ...
  if (!win->window_mask || win->buffer_w != mw || win->shadow_cache_h != sh) {
    ...
    for (int y = 0; y < mh; y++) {
      ...
      for (int x = 0; x < mw; x++) {
        ...
        float dist;
        if (dx > 0 && dy > 0) dist = sqrtf(sqrtf(dx*dx*dx*dx + dy*dy*dy*dy)) - r;
        else dist = (dx > dy ? dx : dy) - r;
...
void kernel_set_hud_status(const char *status) {
  if (!status) return;
  strncpy(g_hud_status, status, 63);
  g_hud_status[63] = '\0';
}

...
通常のUEFI起動では、最初に `baram-nano-system` が起動します。Nano Systemは、UEFI補助機能とウォッチドッグ、最低限のフレームバッファ描画、キーボード、マウス、周期タイマー、ファームウェア経由のリセットを担当します。初期化後は、取得したハードウェア資源の所有権をメインのBaramOSカーネルへ渡します。

現在はNano Systemとメインカーネルを同じUEFIバイナリへリンクしています。入口と所有権の受け渡しは分離してあるため、将来はこの境界をバイナリローダーによるカーネル起動へ置き換えられます。

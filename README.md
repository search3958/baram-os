...
アプリケーションとランタイムデータは `files/app` と `files/data` にまとめています。各ビルドではこれらを FAT ボリューム上の通常のディレクトリ・ファイルとして配置します。`.w3a`・`.w4a`・`.s4a`のアプリディレクトリだけは、実行時のアプリ形式を保つため個別ファイルへパッケージされます。

OS の VFS は `/apps/...`、`/app/...`、`/data/...`、`/files/...` を FAT 上の通常ファイルとして直接読み込みます。`write_file` と削除も同じファイルシステムへ直接反映します。`files.tar`へのフォールバックはありません。

`files/.baram-ignore`の`xiao:`配下に指定したパスは、Xiaoイメージへ含めません。通常OSのイメージにはそのまま含まれます。

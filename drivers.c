...
void exception_handler(struct regs *r) {
  // VGAテキストモードに例外番号を表示（フレームバッファが未初期化でも安全）
  volatile uint16_t *vga = (volatile uint16_t *)0xB8000;
  // 背景赤・文字白で "EXCEPTION: " を表示
  const char *msg = "EXCEPTION: ";
  for (int i = 0; msg[i]; i++)
    vga[i] = (uint16_t)(0x4F00 | (unsigned char)msg[i]);
  // 例外番号を16進で表示
  uint32_t num = r ? (uint32_t)r->int_no : 0xDEAD;
  char hex[9];
  for (int i = 7; i >= 0; i--) {
    int nibble = num & 0xF;
  ...
  hex[8] = 0;
  for (int i = 0; i < 8; i++)
    vga[11 + i] = (uint16_t)(0x4F00 | (unsigned char)hex[i]);
  // フレームバッファが初期化済みなら画面を赤く塗る
  if (g_vram) {
    for (int i = 0; i < SCREEN_WIDTH * SCREEN_HEIGHT; i++)
      g_backbuffer[i] = 0xFFFF0000;
    screen_refresh();
  }
  while (1)

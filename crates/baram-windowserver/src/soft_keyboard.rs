...
keyboard {
candidate-row { button.candidate-mode { text("English") type("primary") } }
...
keyboard-row { button.symbols { text("123") } button.comma { text(",") type("tonal") } button.space { text("Space") type("tonal") } button.period { text(".") type("tonal") } button.close { text("完了") } }
}
"#;
const UPPER: &str = r#"
config { title("Software Keyboard") }
...
config { title("Software Keyboard") }
keyboard {
candidate-row { button.candidate-mode { text("Symbols") type("primary") } }
...
keyboard-row { button.letters { text("ABC") } button.comma { text(",") type("tonal") } button.space { text("Space") type("tonal") } button.period { text(".") type("tonal") } button.close { text("完了") } }
}
"#;
...
impl SoftKeyboard {
    ...
    fn sync_candidate_shelf(&mut self) {
        self.engine
            .set_candidate_items(keyboard_mode_name(self.language), &self.candidates);
    }
    ...
    pub fn click(&mut self, x: i32, y: i32, sw: usize, sh: usize) -> Option<Key> {
        ...
        // A software-keyboard press uses the same visual language as Warp3's
        // hover, but only for this short pressed interval.
        self.engine.set_hover(local_x, local_y);
        self.pressed = true;
        self.pressed_started_ns = None;
        ...
        match id.as_str() {
            ...
            other if other.starts_with("candidate-") => other[10..]
                .parse::<usize>()
                .ok()
    ...
    pub fn tick(&mut self, now_ns: u64) -> bool {
        ...
        if self.pressed {
            let started = *self.pressed_started_ns.get_or_insert(now_ns);
            if now_ns.saturating_sub(started) >= 50_000_000 {
                self.pressed = false;
                self.pressed_started_ns = None;
                self.engine.clear_hover();
                changed = true;
...
fn keyboard_mode_name(language: KeyboardLanguage) -> &'static str {
    match language {
